...
function ensureYtDlpAvailable(): string | null {
  ...
  const possiblePaths = [
    join(resourcesPath, 'app.asar.unpacked', binaryName),
    join(appPath, '..', 'app.asar.unpacked', binaryName),
...
async function ensureFfmpegAvailable(_win?: BrowserWindow): Promise<string | null> {
  ...
  for (const path of possibleFfmpegPaths) {
    if (existsSync(path)) {
      console.log('[FFmpeg] Found binary at:', path)
      return path
    }
  }
  ...
  const ffmpegDir = join(app.getPath('userData'), 'bin')
  if (!existsSync(ffmpegDir)) {
    await fsPromises.mkdir(ffmpegDir, { recursive: true })
  }
  ...
  try {
    ...
    winForDownload?.webContents.send('download-started', {
      ...
      name: 'FFmpeg Dependency (Merging & Quality)',
      size: '~80 MB',
      progress: 0,
      status: 'downloading',
      createdAt: Date.now()
